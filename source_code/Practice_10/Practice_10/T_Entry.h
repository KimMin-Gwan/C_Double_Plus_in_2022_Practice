...
public:
	...
	//overloading--------------------------------------------------------
	bool operator>(const T_Entry<K,V>& right) { return (_key > right.getKey()); } //compare operator overloading
	bool operator>=(const T_Entry<K,V>& right) { return (_key >= right.getKey()); } //compare operator overloading
	bool operator<(const T_Entry<K,V>& right) { return (_key < right.getKey()); } //compare operator overloading
	bool operator<=(const T_Entry<K,V>& right) { return (_key <= right.getKey()); } //compare operator overloading
	bool operator==(const T_Entry<K,V>& right) { return ((_key == right.getKey()) && (_value == right.getValue())); } //compare operator overloading
	T_Entry<K,V>& operator=(T_Entry<K,V>& right); //assign operator overloading
	//function-----------------------------------------------------------
